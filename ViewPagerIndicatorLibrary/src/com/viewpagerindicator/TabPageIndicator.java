...
public class TabPageIndicator extends HorizontalScrollView implements
	 ...
	 * ACTION_CANCEL的产生：
	 * 
	 * down..move..up 正常的事件序列  down..move..cancel
	 * 
	 * 当一个控件响应了down事件和前面几个move事件，在接下来的move事件当中，move事件被父控件拦截下来，造成这个控件的事件不得善终，
	 * 此时就会产生cancel的事件
	 * 
	 * 一般情况：父控件对于down事件来说，都不会进行拦截
	 * 需要一个机制：由子控件通知父控件，通知它接下来的事件别拦截了
	 * 
	 */
	@Override
	public boolean dispatchTouchEvent(MotionEvent ev) {
		int action = ev.getAction();
		/*System.out.println("TabPageIndicator.dispatchTouchEvent.action="
				+ action);*/
		
		if(action == MotionEvent.ACTION_DOWN) {
			getParent().requestDisallowInterceptTouchEvent(true);//请求父控件别拦截事件
		}
